...
class OAuthCallbackSuccessTestCase(unittest.TestCase):
    ...
    def test_oauth_callback_username_collision(self) -> None:
        ...
        # Mock OAuth profile - email prefix will be 'newuser'
        mock_profile = {
            'id': 'google-222',
            'email': 'newuser@example.com',
            'name': 'New User',
        ...
        def mock_fetch_node_side_effect(model_class, constraints):
            ...
            if model_class == models.OAuthIdentity:
                ...
                if (
                    'username' in constraints
                    and constraints['username'] == 'newuser'
                ):
                    return existing_user  # Username collision!
                elif 'email' in constraints:
        ...
        with (
            ...
            # Verify a user was created with modified username
            user_created = next(
                (o for o in created_objects if isinstance(o, models.User)),
                None,
            )
