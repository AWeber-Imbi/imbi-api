...
class SerializationHelpersTestCase(unittest.TestCase):
    """Test the serialization/deserialization helper functions."""

    def test_serialize_props_dicts_to_json(self) -> None:
        from imbi_api.endpoints.third_party_services import (
            _serialize_props,
        )
        ...
        self.assertEqual(
            result['links'],
            json.dumps({'docs': 'https://example.com'}),
        )
        self.assertEqual(
            result['identifiers'],
            json.dumps({'id': 123}),
    ...
    def test_serialize_props_no_dicts(self) -> None:
        from imbi_api.endpoints.third_party_services import (
            _serialize_props,
        )

        props = {'name': 'Test', 'slug': 'test'}
        result = _serialize_props(props)
        self.assertEqual(result, props)

    def test_deserialize_service_json_strings(self) -> None:
        from imbi_api.endpoints.third_party_services import (
        ...
        result = _deserialize_service(record)
        self.assertEqual(
            result['links'],
            {'docs': 'https://example.com'},
        )
        self.assertEqual(result['identifiers'], {'id': 123})

    def test_deserialize_service_none_fields(self) -> None:
        from imbi_api.endpoints.third_party_services import (
            _deserialize_service,
        )

        record = {'name': 'Test', 'links': None, 'identifiers': None}
        result = _deserialize_service(record)
        self.assertEqual(result['links'], {})
        self.assertEqual(result['identifiers'], {})

    def test_deserialize_service_invalid_json(self) -> None:
        from imbi_api.endpoints.third_party_services import (
            _deserialize_service,
        )
    ...
    def test_serialize_app_props(self) -> None:
        from imbi_api.endpoints.third_party_services import (
            _serialize_app_props,
        )
        ...
        result = _serialize_app_props(props)
        self.assertEqual(
            result['scopes'],
    ...
    def test_deserialize_app_json_strings(self) -> None:
        from imbi_api.endpoints.third_party_services import (
            _deserialize_app,
        )
        ...
        result = _deserialize_app(record)
        self.assertEqual(result['scopes'], ['read', 'write'])
        self.assertEqual(result['settings'], {'debug': True})

    def test_deserialize_app_none_fields(self) -> None:
        from imbi_api.endpoints.third_party_services import (
            _deserialize_app,
        )
    ...
    def test_deserialize_app_invalid_json(self) -> None:
        from imbi_api.endpoints.third_party_services import (
            _deserialize_app,
        )
