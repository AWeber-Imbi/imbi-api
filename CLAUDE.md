...
```bash
AGE_URL=postgresql://postgres:secret@localhost:5432/imbi
```
...
just test tests/age/test_client.py

# Run specific test class or method
just test tests/age/test_client.py::AGEClientTestCase
just test tests/age/test_client.py::AGEClientTestCase::test_singleton
...
- **`imbi_common` package** (external dependency): Shared code used across Imbi services
  ...
  - `age/`: Apache AGE graph database integration layer
    - `client.py`: Singleton driver with event loop awareness
    - `__init__.py`: High-level API and graph relationship helpers
    - `constants.py`: Index definitions and configuration
...
### AGE Integration Pattern

The AGE module uses a **singleton pattern with event loop awareness**:

```python
from imbi_common import age

# Module-level APIs (preferred):
async with age.session() as sess:
    # Use session
    pass

async with age.run('MATCH (n) RETURN n', param=value) as result:
    records = await result.data()

# High-level operations:
await age.initialize()  # Set up indexes
vertex_id = await age.upsert(node_model, {'id': '123'})
await age.aclose()  # Cleanup

# Graph relationship helpers (type-safe Pydantic integration):
node_id = await age.create_node(model_instance)  # Create node from model
edge_id = await age.create_relationship(
    source_model, target_model, rel_type='DEPENDS_ON'
)
await age.refresh_relationship(model, 'dependencies')  # Lazy-load relationships
edges = await age.retrieve_relationship_edges(model, 'dependencies')
```

**Implementation details** (`imbi_common/age/client.py`):
- `AGE.get_instance()`: Returns singleton driver instance
- Automatically reinitializes if event loop changes (important for FastAPI)
- Manages AGE connection pool with keep-alive and max connection settings
- `initialize()` creates indexes defined in `age/constants.py`

**Upsert pattern** (`imbi_common/age/__init__.py:upsert()`):
- Uses openCypher `MERGE` with `ON CREATE SET` and `ON MATCH SET`
- Takes constraint dict for matching (e.g., `{'id': '123'}`)
- Automatically maps Pydantic model properties to vertex properties
- Returns AGE vertex id of created/updated node

**Graph relationship helpers** (`imbi_common/age/__init__.py`):
- `create_node()`: Create AGE vertices from Pydantic models with automatic label/property mapping
- `create_relationship()`: Create typed relationships between vertices with optional properties
...
2. `age_hook` — init/close AGE connection pool
3. `age_setup_hook` — create API-specific indexes, refresh blueprint models for OpenAPI
...
Hooks that yield resources (email, storage) use the `Lifespan.get_state()` DI pattern — see "Dependency Injection Pattern" below. AGE and ClickHouse still use module-level singletons in `imbi_common` and are not yet DI'd.
...
**Not yet DI'd**: AGE and ClickHouse still use module-level singletons in `imbi_common`. Full DI for these requires refactoring the shared library to expose client objects.
...
1. **Pydantic models** (`imbi_common/models.py`):
   ...
   - Model class names become AGE vertex labels (lowercase)
...
2. **Settings** (`imbi_common/settings.py`):
   ...
   - Prefix environment variables (e.g., `AGE_URL`, `CLICKHOUSE_URL`)
   - Support `.env` files
   - Separate settings classes for auth, AGE, ClickHouse, server, email
...
class MyTestCase(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        # Reset singleton for test isolation (AGE, ClickHouse)
...
✅ **Implemented**:
- FastAPI application with lifespan management (Email, Storage DI-managed via lifespan hooks; AGE, ClickHouse remain module-level singletons)
- Status endpoint with health check (`GET /status`)
- CLI with `serve` and `setup` commands (development and production modes)
- AGE integration with singleton pattern, graph relationship helpers, indexes, upsert operations
